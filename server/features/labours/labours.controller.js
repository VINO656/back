...
exports.createLabour = async (req, res, next) => {
  try {
    ...
    const created = await Model.create({ ...data, createdBy: req.user?.id });
...
exports.updateLabour = async (req, res, next) => {
  try {
    ...
    if (!old) return res.status(404).json({ ok: false, message: 'Labour record not found' });
...
exports.deleteLabour = async (req, res, next) => {
  try {
    ...
    res.json({ ok: true, message: 'Labour record and associated user deleted successfully' });
  } catch (e) { next(e); }
};
