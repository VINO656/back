...
async function createSaleFromInvoice(inv) {
  ...
  for (const it of (inv.items || [])) {
    ...
    if (it.inventoryId) {
      ...
      if (stock && stock.sourceType === 'Purchase' && stock.sourceRef) {
...
exports.updateInvoiceStatus = async (req, res, next) => {
  try {
    if (req.user?.role !== 'Admin') {
      return res.status(403).json({ ok: false, message: 'Only Admin can approve or reject invoices' });
    }
...
exports.updateInvoice = async (req, res, next) => {
  try {
    ...
    if (!old) return res.status(404).json({ ok: false, message: 'Invoice not found' });
    if (old.payStatus === 'Paid' && req.user?.role !== 'Admin') {
      return res.status(403).json({ ok: false, message: 'BR-10: Paid invoice cannot be modified without prior Admin approval.' });
    }
    const cleanBody = sanitizeUpdate(req.body);
...
exports.deleteInvoice = async (req, res, next) => {
  try {
    if (req.user?.role !== 'Admin') {
      return res.status(403).json({ ok: false, message: 'Only Admin can delete invoices' });
    }
    const inv = await Invoice.findById(req.params.id);
    if (inv) {
